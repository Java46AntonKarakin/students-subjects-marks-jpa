...
public interface MarkRepository extends JpaRepository<MarkEntity, Long> {
	...
	@Query(value = "select name, subject, mark" + STUDENTS_SUBJECTS_MARKS + "where name=:name", nativeQuery = true)
	List<StudentSubjectMark> findByStudentName(String name);

	@Query(value = "select name, round(avg(mark)) as avgMark " + STUDENTS_MARKS + "group by name", nativeQuery = true)
	...
	@Query(value = "select name " + STUDENTS_MARKS
			+ "group by name order by avg(mark) desc limit :nStudents", nativeQuery = true)
	...
	@Query(value = "select name, subject, mark " + STUDENTS_SUBJECTS_MARKS + " where subject=:subject "
			+ "group by name order by avg(mark) asc limit :nStudents", nativeQuery = true)
	List<StudentSubjectMark> worstStudentsMarks(int nStudents);

	@Query(value = "select " + MIN_INTERVAL + " as min," + MIN_INTERVAL + " + :interval - 1 as max, "
